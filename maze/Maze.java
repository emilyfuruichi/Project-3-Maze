...
public class Maze 
{
	...
	public Maze(int size)
	{
		...
		Map<Integer,Cell> solution, solutionShort;
...
//		System.out.println(shortestGrid);
	 ...
	 */
	private Map<Integer,Cell> solveMazeDFS(String kindOfPath) 
	{
		...
		while(currentCell.getCellNum() != numOfCells-1) //maze not solved
		{
			...
			else
			{
				...
				//if(kindOfPath.equals("shortest") && sol.containsKey(currentCell))
					//sol.remove(--order);
	...
	*/
	...
	public String printSolutionSummary(Map<Integer,Cell> allVisitedMap, Stack<Integer> shortestPath)
	{
		...
		sol += "Path: ";
		for(Map.Entry<Integer, Cell> m : allVisitedMap.entrySet())
		{
			sol += "("+m.getValue().getX()+","+m.getValue().getY()+") ";
		}
		sol += "\nLength of path: " ;//+ shortestPath.size(); //all cells visited
		sol += "\nVisited cells: "+ allVisitedMap.size(); //shortest path length
